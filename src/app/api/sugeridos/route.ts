...
export async function GET(request: Request) {
  ...
  for (const r of tmdbResults) {
    ...
    if (film) { results.push({ ...film, media_type: 'movie', tipo: 'filme' }); continue; }
    ...
    if (serie) { results.push({ ...serie, media_type: 'tv', tipo: 'serie' }); continue; }
    ...
    if (anime) { results.push({ ...anime, media_type: 'tv', tipo: 'anime' }); continue; }
  }
